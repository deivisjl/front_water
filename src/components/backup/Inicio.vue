        ...
        <v-data-table
              ...
              <template v-slot:no-results>
                ...
                <template v-slot:top>
                    ...
                    <v-toolbar-title>Respaldos de información</v-toolbar-title>
                    <v-spacer></v-spacer>
                    ...
                    <v-btn
                        ...
                        class="mb-2"
                        @click="nuevo()"
                    >{{ $t('miscelanius_new_backup')}}</v-btn>
                    </v-toolbar>
                    <!--  -->
                        <v-card
                          ...
                          <v-toolbar dense>
                            <v-tooltip top>
                              ...
                              <v-tooltip top v-if="unitario">
                                <template v-slot:activator="{ on, attrs }">
                                  <v-btn icon v-bind="attrs" v-on="on" @click="descargar_archivo(selected[0])">
                                    <v-avatar color="primary" size="36">
                                      <v-icon color="white darken-2">get_app</v-icon>
                                    </v-avatar>
                                  </v-btn>
                                </template>
                                <span>{{ $t('miscelanius_download_item') }}</span>
                              </v-tooltip>
                              <v-icon v-if="unitario">more_vert</v-icon>
                              <v-tooltip top v-if="opciones">
                                <template v-slot:activator="{ on, attrs }">
                                  <v-btn icon v-bind="attrs" v-on="on"  @click="eliminar_respaldo(selected[0])">
...
export default {
  ...
  methods:{
    ...
    {
       this.loader = true
            this.$store.state.services.backupService
              .downloadBackup(item.id)
              .then((r)=>{
                  this.loader = false
    ...
    nuevo(){
        this.$swal({
		          title: "¿Esta seguro de realizar esta tarea?",
                  text: "Respaldo de información de base de datos",
                  icon: "question",
