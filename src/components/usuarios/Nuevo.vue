            ...
            <v-col md="8" offset="2">
                <v-card>
                    ...
                    <v-card-text style="margin-top:10px">
                         <!--  -->
                        <v-stepper v-model="e6" vertical style="box-shadow: none">
                            ...
                            <v-stepper-content step="1">
                               <v-card>
                                 <v-card-text>
                                    <form autocomplete="off" @submit.prevent="validar_datos_formulario('datos_generales',2)" data-vv-scope="datos_generales">
                                      ...
                                      <v-btn color="primary" type="submit">{{ $t('miscelanius_next_item') }}</v-btn>
                                    </form>
                                 </v-card-text>
                               </v-card>
                            </v-stepper-content>
                            <!-- Step 2 -->
                            <v-stepper-step :complete="e6 > 2" step="2">Datos para notificación</v-stepper-step>

                            <v-stepper-content step="2">
                              <v-card>
                                  <v-card-text>
                                    <form autocomplete="off" @submit.prevent="validar_datos_formulario('notificacion',3)" data-vv-scope="notificacion">
                                      ...
                                      <v-btn color="primary" type="submit">{{ $t('miscelanius_next_item') }}</v-btn>
                                      <v-btn text @click="e6 = 1">{{ $t('miscelanius_previuos_item') }}</v-btn>
                                    </form>
                                  </v-card-text>
                              </v-card>
                            </v-stepper-content>
                            <!--  end step 2-->
                            <v-stepper-step step="3">Datos de la cuenta</v-stepper-step>
                            
                            <v-stepper-content step="3">
                              <!--  -->
                              <v-card>
                                  <v-card-text>
                                      <form autocomplete="off" @submit.prevent="validar_datos_formulario('cuenta', 4)" data-vv-scope="cuenta">
                                        <v-text-field outlined dense name="dpi" v-model="model.dpi" v-validate="'required|numeric|min:13'" label="DPI"></v-text-field>
                                        ...
                                        <v-btn text @click="e6 = 2">{{ $t('miscelanius_previuos_item') }}</v-btn>
                                      </form>
                                  </v-card-text>
                              </v-card>
                            </v-stepper-content>
...
import FormError from "@/components/shared/FormError"
  ...
  export default {
    ...
    methods:{
      validar_datos_formulario(scope, next){
          this.$validator.validateAll(scope).then((result) => { 
              if(result)
              {
                (next === 4) ? this.guardar() : (this.e6 = next)
              }
          });
      },
      guardar(){
