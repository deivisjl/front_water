                        ...
                        <v-card
                          ...
                          <v-toolbar dense>
                            <v-tooltip top>
                              ...
                              <v-tooltip top v-if="unitario">
                                <template v-slot:activator="{ on, attrs }">
                                  <v-btn icon v-bind="attrs" v-on="on" @click="aprobar_solicitud(selected[0])">
                                    <v-avatar color="primary" size="36">
                                      <v-icon color="white darken-2">done</v-icon>
                                    </v-avatar>
                                  </v-btn>
                                </template>
                                <span>{{ $t('miscelanius_approve_item') }}</span>
                              </v-tooltip>
                              <v-icon v-if="unitario">more_vert</v-icon>
                              <v-tooltip top v-if="opciones">
                                <template v-slot:activator="{ on, attrs }">
                                  <v-btn icon v-bind="attrs" v-on="on"  @click="rechazar_solicitud(selected[0])">
                                    <v-avatar color="deep-orange accent-3" size="36">
                                      <v-icon color="white darken-2">clear</v-icon>
                                    </v-avatar>
                                  </v-btn>
                                </template>
                                <span>{{ $t('miscelanius_reject_item') }}</span>
                              </v-tooltip>
                          </v-toolbar>
                        </v-card>
                    <!--  -->
...
export default {
  ...
  methods:{
    ...
    editar_estado(data){
      if(data)
      {
        this.$router.push({path:`solicitudes/editar/`+data.id});
      }
    },
    rechazar_solicitud(data){
      if(data)
      {
        this.$router.push({path:`solicitudes/rechazar/`+data.id});
      }
    },
    aprobar_solicitud(data){
      if(data)
      {
        this.$router.push({path:`solicitudes/aprobar/`+data.id});
      }
    },
  },
  computed:{
